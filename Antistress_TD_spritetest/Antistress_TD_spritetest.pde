  ...
  /*if (frameCount == 1) {
   text("Loading...", 200, 200);
    ...
    if (towerDragStatus > -1) {
      ...
      case 0:
        pushMatrix();
        translate(mouseX-10, mouseY-50);
        scale(-1, 1);
        image(fighterSprite[0], 0, 0);
        popMatrix();
        break;
      case 1:
        fill(0, 255, 0);
    ...
    //musens markør
    if (hovering()) image(handCursor, mouseX+9, mouseY+16);
    else image(arrowCursor, mouseX+16, mouseY+16);
...
void createSprites() {
  PImage spriteSheet = loadImage("cannon.png");
  fighterSprite = cutSpriteSheet(spriteSheet, 6, 4, 22);
}
